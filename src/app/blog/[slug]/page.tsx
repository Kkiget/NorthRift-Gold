...
export default function BlogPost() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <Link href="/blog" className="text-green-600 hover:text-green-700 mb-8 inline-flex items-center">
          <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
          </svg>
          Back to Blog
        ...
        <article className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="relative h-[500px]">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8">
              <div className="mb-4">
                <span className="inline-block bg-white/90 text-green-800 text-sm font-semibold px-4 py-1.5 rounded-full">
                  {post.category}
                </span>
              </div>
              ...
              {post.content.split('\n\n').map((paragraph, index) => (
                <div key={index} className="mb-8">
                  {paragraph.split('\n').map((line, lineIndex) => {
                    if (line.includes(':')) {
                      return (
                        <h2 key={lineIndex} className="text-3xl font-bold text-gray-900 mt-12 mb-6 pb-4 border-b border-gray-100">
                          {line}
                        </h2>
                      );
                    }
                    return (
                      <p key={lineIndex} className="text-gray-700 leading-relaxed mb-6 text-lg">
                        {line}
                      </p>
                    );
                  })}
